## effect.sizes=x,y
plot.meta2 <- function(x, effect.sizes, add.margin=0.1, interval=0.95,
  ...
  if (length(effect.sizes)>2) {
    ...
    ellipse.pt <- lapply(split(x$data, 1:nrow(x$data)),
                         ...
                         ellipse( vec2symMat(rand.val),
                                  centre=c(exp(x[effect.sizes[1]]),exp(x[effect.sizes[2]])))})
    ...
    my.x <- exp(x$data[complete, effect.sizes[1]])
    my.y <- exp(x$data[complete, effect.sizes[2]])
